...
func init(_config: CubeEnemyConfig, _view: CubeEnemyView) -> void:
	...
	view.hit_area.connect("area_entered", self.on_body_area_entered)
	pass
...
func on_body_area_entered(area: Area2D):
	if area is AttackArea and area.group == EntityGroups.Groups.PLAYER:
		take_damage(area.damage)
		pass
	pass

func take_damage_tween():
